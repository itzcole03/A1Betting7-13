    ...
    async def _make_api_request(
        ...
        for attempt in range(self.max_retries):
            try:
                ...
                logger.error(
                    f"❌ HTTP error {e.response.status_code} for URL {e.request.url}: {e}"
                )
                if e.response.status_code in [429, 500, 502, 503, 504]:
                    if attempt < self.max_retries - 1:
                        wait_time = self.base_backoff_delay * (2**attempt)
                        logger.info(
                            f"🔄 Retrying in {wait_time}s due to server error..."
                        )
                        await asyncio.sleep(wait_time)
                        continue
                return None
                ...
                if attempt < self.max_retries - 1:
                    wait_time = self.base_backoff_delay * (2**attempt)
                    logger.info(
                        f"🔄 Retrying in {wait_time}s due to unexpected error..."
                    )
                    await asyncio.sleep(wait_time)
                    continue
                return None
