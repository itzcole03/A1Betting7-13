"""
Enhanced PrizePicks Service v2 with ML Integration
Production-ready service for fetching actual PrizePicks props with ML ensemble predictions
"""
...
class EnhancedPrizePicksService:
    """Enhanced PrizePicks service with real data integration and ML predictions"""
    ...
    async def initialize(self) -> bool:
        """Initialize the HTTP client and ML ensemble service"""
        try:
            headers = {
            ...
            if ML_ENSEMBLE_AVAILABLE:
                try:
                    ml_success = await enhanced_ml_ensemble_service.initialize()
                    if ml_success:
                        logger.info("✅ ML ensemble service initialized")
                    else:
                        logger.warning("⚠️ ML ensemble service initialization failed")
                except Exception as e:
                    logger.warning(f"⚠️ ML ensemble service error: {e}")
    ...
    async def fetch_projections_api(self) -> List[Dict[str, Any]]:
        ...
        try:
            ...
            logger.warning("⚠️ All API endpoints failed, returning enhanced mock data")
            return await self._get_enhanced_mock_data()
    ...
    async def _get_enhanced_mock_data(self) -> List[Dict[str, Any]]:
    ...
    async def _enhance_props_with_ml(self, props: List[Dict[str, Any]]) -> List[Dict[str, Any]]:
        ...
        try:
            if not ML_ENSEMBLE_AVAILABLE:
                logger.info("📊 ML ensemble not available, returning original props")
                return props
    ...
    async def scrape_prizepicks_props(self) -> List[Dict[str, Any]]:
        """Main method to fetch PrizePicks props with ML enhancements"""
        try:
            # Check cache first
            ...
            # Enhance props with ML predictions
            if ML_ENSEMBLE_AVAILABLE:
                props = await self._enhance_props_with_ml(props)
            else:
                logger.info("📊 ML enhancement skipped - service not available")
            
            # Cache the results
            self.cache[cache_key] = (props, time.time())
            
            logger.info(f"🏆 Successfully fetched {len(props)} PrizePicks props with ML enhancements")
            return props
    ...
    def get_scraper_health(self) -> Dict[str, Any]:
        ...
        try:
            ...
            ml_status = "available" if ML_ENSEMBLE_AVAILABLE else "not_available"
...
enhanced_prizepicks_service_v2 = EnhancedPrizePicksService()


async def start_enhanced_prizepicks_service_v2():
    """Start the enhanced PrizePicks service v2"""
    try:
        success = await enhanced_prizepicks_service_v2.initialize()
        if success:
            logger.info("🚀 Enhanced PrizePicks service v2 started successfully")
        else:
            logger.error("❌ Failed to start enhanced PrizePicks service v2")
        return success
    except Exception as e:
        logger.error(f"❌ Exception starting enhanced PrizePicks service v2: {e}")
        return False
